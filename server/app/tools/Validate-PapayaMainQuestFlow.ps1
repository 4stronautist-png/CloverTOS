...
$criticalChecks = @{
    ...
    'Gele Plateau Grown Apart From Hope boss track requires a killable Biteregina objective' = $content.Quests -match 'GELE574_MQ_09[\s\S]*objectives:\s*\[\{[^\]]*type:\s*"Kill"[^\]]*target:\s*"boss_BiteRegina_Q3"' -and $content.SessionObjects -match 'GELE574_MQ_09[\s\S]*monsterNameGroup:\s*\[\s*"boss_BiteRegina_Q3"\s*\]' -and $content.PrivateEncounters -match '"questName"\s*:\s*"GELE574_MQ_09"[\s\S]*"target"\s*:\s*"boss_BiteRegina_Q3"[\s\S]*"f_gele_57_4 1287 -78 1811 350"'
